...
; plus: the first four also name rows of StepVectors (see
; engine/overworld/map_objects.asm), which indexes with two bits, so those four
; have to stay at 0-3. STEP_RUN takes the row vanilla leaves empty; STEP_BIKE
; and everything above it in the table keep their meaning, and the constants
; below STEP_RUN, which are only ever DoStep arguments, shift up by one.
	const_def
	const STEP_SLOW          ; 0
	const STEP_WALK          ; 1
	const STEP_BIKE          ; 2
