...
; Sizes 79/44/27.
;
; POOL (150 species): everything with a wild source in vanilla Crystal
; (grass, water, swarms, fishing, headbutt/rock smash, bug contest) plus
; Game Corner prize mons and the three starters, minus the 11 legendaries
; and Unown.
;
; The pool is a permutation domain: every member lands in some wild slot,
; so the dex stays completable. The other 89 species have no wild source
; in vanilla either, so nothing is lost.
;
...
; is always a basic-stage mon that can still evolve.
;
; NOTE: Abra, Cubone, Magikarp, Ditto, Hoppip and Smeargle are in C tier
; but NOT starter-legal - none of them can deal damage at level 5, which
; would make the first rival battle unwinnable.
