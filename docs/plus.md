...
| One-line battle HUD | always on | done |
...
| `tools/build_vanilla.sh`, `tools/make_bps.py`, `tools/make_half_font.py` | vanilla reference build, patch generation, the condensed HUD font |
...
| `gfx/font/font_half.png` | whole file | HUD | generated by `tools/make_half_font.py`; the wide glyphs are copied from the game's own graphics |
| `engine/battle/core.asm` | `DrawEnemyHUD`, `PrintPlayerHUD` | HUD | the name row comes down onto the bar and is written from `PlusHUDEnemyTiles` / `PlusHUDPlayerTiles` |
| `engine/battle/trainer_huds.asm` | `DrawEnemyHUDBorder` | HUD | the caught mon marker moves up a row, out of the name's way |
